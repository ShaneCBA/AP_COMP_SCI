...
public class IntegerListS
{
    ...
    // -----------------------------------------------------------------
    private int linearSearchR (int target, int lo)//lo==location
    {
		System.out.println(list[lo]);
		if (target > list[list.length-1] || target < list[0]) return -1;
		if (list[lo]==target)return lo;
		if (list[lo]!=target) return linearSearchR(target, lo+1);
		return -1;
    }
